class Swipe {
    ...
    run() {
        this.element.addEventListener('touchmove', function(evt) {
			try{this.handleTouchMove(evt).bind(this);}
			catch(err){
				console.log(err);
			}
...
$(document).ready(function(){
	...
	function overlaySystem(sectionContainerPT,sectionsPT,frontpagePT,frontpageContainerPT) {
		...
		(new Swipe('body')).onUp(function() { nextOverlay(); }).onDown(function() { prevOverlay(); }).run();
		...
		switch(option_overlay_content_animation){
			...
			break;
			case 'slide':
				sections.addClass("slide-from-right");
			break;
			default:
				sections.addClass("slide-from-right");
			break;
		  ...
		  $.ajax({
			  ...
			  success: function(json) {
				  ...
				  if(json.valid === 0) {
					  ...
					  $('.invalid_Email_Error').css("display", "block");
					  $('.subscribe-form input').val('');
					  $('.subscribe-form-div').css("margin-bottom", "30px");
					  //$('.subscribe-form input').attr("placeholder", json.message);

                      $('.invalid_Email_Error span[data-error]').html('Please enter a valid E-mail Address');
                      setTimeout(function(){
						...
						$('.invalid_Email_Error span[data-error]').html('Email already exist!');
					  ...
					  $('.sub_heading_large').css("display", "none");
					  $('.sub_heading_small').css("display", "none");
				  }
			  }
