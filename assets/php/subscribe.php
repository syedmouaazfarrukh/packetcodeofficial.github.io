...
// ENTER NAME OF FILE 
$already_subscriber_list = "already-subscriber-list.txt";
...
// ENTER ALREADY EXIST MESSAGE
$varAlreadyExist = "* Email already exist!";


function emailWriteToFile($emailAddress) {
    $myfile = fopen($GLOBALS['already_subscriber_list'], "a") or die("Unable to open file!");
    fwrite($myfile, strtolower($emailAddress)."\n");
    fclose($myfile);
  }

function checkEmailFromFile($emailAddress) {
    $myfile = fopen($GLOBALS['already_subscriber_list'], "r") or die("Unable to open file!");
    $emailList = "";

    while(! feof($myfile))
    {
        $emailList =  $emailList.fgets($myfile).",";
    }
    fclose($myfile);
    if (strpos($emailList, strtolower($emailAddress)) != null)  
        return false;
    else
        return true;
  }
...
if ($_POST) {
    ...
    if ($subscriber_email == "") {
        ...
        if (!filter_var($subscriber_email, FILTER_VALIDATE_EMAIL)) {
            ...
            if(checkEmailFromFile($subscriber_email)){
                if ($mode === "file") {
                    ...
                    if (file_exists($file_path . $file_name)) {
                        ...
                        emailWriteToFile($subscriber_email);
                    } else {
